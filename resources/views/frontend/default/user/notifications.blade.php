                                        ...
                                        <li
                                            ...
                                            <a href="{{ url($notification->link) }}" class="media text-inherit">
                                                <span class="avatar avatar-sm mr-3">
                                                    <img
                                                        src="@if (!empty($notification->sender->photo)) {{ asset('profile/photos/' . $notification->sender->photo) }}
                                                    @else
                                                        {{ asset('assets/backend/default/img/avatar-place.png') }} @endif">
                                                </span>
                                                <div class="media-body">
                                                    <p class="mb-1">{{ $notification->message }}
                                                        {{ $notification->sender_id > 0 ? $notification->sender->name : '' }}
                                                    </p>
                                                    <small
                                                        class="text-muted">{{ Carbon::parse($notification->created_at)->diffForHumans() }}</small>
                                                </div>
                                            </a>
