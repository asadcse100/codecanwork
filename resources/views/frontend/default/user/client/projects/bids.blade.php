                        ...
                        @forelse ($bid_users as $key => $bid_user)
                            <div class="card">
                                <div class="card-body d-lg-flex">
                                        ...
                                        <div class="d-flex">
                                            <a href="" target="_blank" class="avatar flex-shrink-0 mr-4">
                                                @if ($bid_user->expert->photo != null)
                                                    <img
                                                        src="{{ asset('profile/photos/' . $bid_user->expert->photo) }}">
                                                @else
                                            ...
                                            <div class="flex-grow-1">
                                                <h5 class="fw-600 mb-1"><a href="" target="_blank"
                                                        class="text-inherit">{{ $bid_user->expert->name }}</a></h5>
                                                @if ($bid_user->expert->profile->specialist != null)
                                                    <p class="opacity-50 mb-2">
                                                        {{ $bid_user->expert->profile->specialistAt->name }}</p>
                                                @endif
                                                <div class="d-flex text-secondary fs-12 mb-3">
                                                    <div class="mr-2">
                                                        ...
                                                        <span>
                                                            ({{ getNumberOfReview($bid_user->bid_by_user_id) }}
                                                            {{ translate('Reviews') }})
                                                        </span>
                                                    </div>
                                                    <div>
                                                        <i class="las la-map-marker opacity-50"></i>
                                                        @if ($bid_user->expert->address != null &&
                                                            $bid_user->expert->address->city != null &&
                                                            $bid_user->expert->address->country != null)
                                                            <span>{{ $bid_user->expert->address->city->name }},
                                                                {{ $bid_user->expert->address->country->name }}</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div
                                        class="flex-shrink-0 pt-4 pt-xl-0 pl-xl-5 d-flex flex-row flex-xl-column justify-content-between">
                                        ...
                                        <div>
                                            @if ($project->biddable == 1)
                                                <button
                                                    ...
                                                    type="button" class="btn btn-outline-secondary btn-sm btn-block">Hire
                                                    now</button>
                                                <form class="mt-2" action="{{ route('call_for_interview') }}"
                                                    ...
                                                    <input type="hidden" id="user_name" name="user_name"
                                                        value="{{ $bid_user->expert->user_name }}">
                                                    <button type="submit"
                                                        class="btn btn-primary btn-sm btn-block">{{ translate('Call for Interview') }}</button>
                                                </form>
                                            @elseif($project->project_user != null && $project->project_user->user_id == $bid_user->bid_by_user_id)
                                                <button type="button"
                                                    class="btn btn-primary btn-sm btn-block">{{ translate('Hired') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
...
@section('modal')
    <div class="modal fade" id="hiring_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body" id="hiring_modal_body">
                    <form class="form-horizontal" action="{{ route('hiring_confirmation_store') }}" method="POST"
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-group">
                                <input type="number" class="form-control form-control-sm" name="amount" value=""
                                    min="1" step="0.01" required>
                            </div>
                        </div>
                        <div class="form-group text-right">
                            <button type="submit"
                                class="btn btn-sm btn-primary transition-3d-hover mr-1">{{ translate('Hire Now') }}</button>
                        </div>
                    </form>
                </div>
            </div>
