                        ...
                        <div class="media mb-3 border-bottom">
                            <div class="media-body">
                                ...
                                <ul class="list-inline font-size-1 text-muted">
                                    <li class="list-inline-item">
                                        ...
                                        <span
                                            class="align-middle">{{ Carbon\Carbon::parse($project->created_at)->diffForHumans() }}</span>
                                    </li>
                                    <li class="list-inline-item">
                            ...
                            <div class="d-flex ml-auto">
                                ...
                                <div class="position-relative d-inline-block mr-2">
                                    ...
                                    <span
                                        class="align-middle font-size-2 font-weight-medium">{{ single_price($project->price) }}</span>
                                </div>
                                <!-- End budget -->
                            </div>
                        </div>
                            ...
                            @foreach (json_decode($project->attachment) as $key => $attachment_id)
                                ...
                                @if ($attachment != null)
                                    <div class="d-flex justify-content-between align-items-center mt-2 file-preview-item"
                                        data-id="29" title="fsfhjllz_vegetables-banner.png">
                                        <div
                        ...
                        @if ($project->client_user_id != Auth::user()->id)
                            <div class="mb-5 text-center">
                                <a class="btn btn-block btn-sm btn-primary transition-3d-hover"
                                    onclick="confirm_invite_modal({{ $project->id }})">{{ translate('Invitation Reply') }}</a>
                            </div>
                        @endif
                        @if ($chat_thread != null)
                            <div class="mb-5 text-center">
                                <a class="btn btn-block btn-sm btn-primary transition-3d-hover"
                                    href="{{ route('chat_view', $chat_thread->id) }}">{{ translate('Go for Chat') }}</a>
                            </div>
                        @endif
                        <!-- Sidebar Info -->
                        <div class="card shadow-soft shadow-sm mb-4">
                            ...
                            <div class="card-body pt-1 px-5 pb-5">
                                ...
                                <div class="media mb-3">
                                    ...
                                    <div class="media-body">
                                        <span
                                            class="d-block font-weight-medium">{{ count(\App\Models\Project::where('client_user_id', $project->client_user_id)->get()) }}
                                ...
                                <div class="media mb-3">
                                    ...
                                    <div class="media-body">
                                        <span
                                            class="d-block font-weight-medium">{{ translate('$30.91/hr avg hourly rate paid') }}</span>
...
@section('script')
    <script type="text/javascript">
        function confirm_invite_modal(id) {
            $.post('{{ route('get_invitation_reply_modal') }}', {
                _token: '{{ csrf_token() }}',
                id: id
            }, function(data) {
...
@endsection
@section('modal')
    <div class="modal fade" id="invitation_reply" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
