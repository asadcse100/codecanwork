...
@section('content')
    <div class="h-250px">
        @if ($client->cover_photo != null)
            <img src="{{ asset('profile/cover_photo/' . $client->cover_photo) }}"
                alt="{{ $client->name }}"class="img-fit h-250px">
        @else
            <img src="{{ asset('assets/frontend/default/img/cover-place.jpg') }}"
                alt="{{ $client->name }}"class="img-fit h-250px">
        @endif
    </div>
    <div class="mt-n5">
                    ...
                    <div class="media align-items-center d-block d-md-flex">
                        <div class="mr-5 text-center text-md-left mb-4 mb-md-0">
                            <span class="avatar avatar-xxl">
                                @if ($client->photo != null)
                                    <img src="{{ asset('profile/photos/' . $client->photo) }}">
                                @else
                        ...
                        <div class="media-body d-lg-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        ...
                                        @if ($client->address != null && $client->address->city != null && $client->address->country != null)
                                            <span>{{ $client->address->city->name }},
                                                {{ $client->address->city->country->name }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-lg-right text-center">
                                @if (Auth::check() &&
                                    ($bookmarked_client = \App\Models\BookmarkedClient::where('user_id', auth()->user()->id)->where('client_user_id', $client->id)->first()) != null)
                                    <a class="btn btn-secondary confirm-alert" href="javascript:void(0)"
                                        data-href="{{ route('bookmarked-clients.destroy', $bookmarked_client->id) }}"
                                        data-target="#unfollow-modal">Unfollow</a>
                                @else
                                    <a class="btn btn-primary"
                                        href="{{ route('bookmarked-clients.store', encrypt($client->id)) }}">{{ translate('Follow') }}</a>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            <div class="row">
                ...
                <div class="col-xxl-9 col-lg-8 order-1 order-lg-0">
                                                ...
                                                <div class="row">
                                                    <div class="col-8">
                                                        <h5 class="fs-14 fw-600 lh-1-5">
                                                            <a href="{{ route('project.details', $open_project->slug) }}"
                                                                class="text-inherit">{{ $open_project->name }}</a>
                                                        </h5>
                                                        <ul class="list-inline opacity-70 fs-12">
                                                            <li class="list-inline-item">
                                                                <i class="las la-clock opacity-40"></i>
                                                                <span>{{ Carbon\Carbon::parse($open_project->created_at)->diffForHumans() }}
                                                                </span>
                                                            </li>
                                                            <li class="list-inline-item">
                                                    ...
                                                    <div class="col-4 text-right">
                                                        <span
                                                            class="h5">{{ single_price($open_project->price) }}</span>
                                                    </div>
                                                </div>
                                            </li>
                                        @endforeach
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p class="text-muted mb-4">{{ translate('Showing') }} {{ getNumberOfReview($client->id) }}
                                {{ translate('reviews') }}</p>
                                            ...
                                            <div class="media">
                                                <div class="mr-3">
                                                    <span class="avatar avatar-md m-3">
                                                        <img
                                                            src="{{ asset('profile/photos/' . \App\Models\User::find($review->reviewer_user_id)->photo) }}">
                ...
                <div class="col-xxl-3 col-lg-4">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="">
                                <h6 class="separator text-left mb-4 fw-600"><span
                                        class="bg-white pr-3">{{ translate('Verifications') }}</span></h6>
                                <div>
                                    <ul class="list-unstyled">
                                        @php
                                            $verification = \App\Models\Verification::where('user_id', $client->id)
