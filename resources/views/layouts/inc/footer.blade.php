<div class="card">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-6 text-right">
                            <div class="dropdown dropup d-inline-block ml-auto">
                                ...
                                @endphp
                                @if ($lang != null)
                                <a class="dropdown-toggle text-secondary no-arrow py-2" data-toggle="dropdown" href="javascript:void(0);" role="button" aria-haspopup="false" aria-expanded="false">
                                    <img src="{{ asset('assets/frontend/default/img/flags/' . $lang->code . '.png') }}" height="11">
    ...
    <div class="aiz-mobile-bottom-nav d-xl-none fixed-bottom border-top rounded-top bg-white shadow-lg" style="box-shadow: 0px -1px 10px rgb(0 0 0 / 15%)!important;">
        <div class="d-flex justify-content-around align-items-center">
            <a href="{{ route('home') }}" class="text-reset flex-grow-1 pb-2 pt-3 text-center">
                ...
                <span class="d-block fs-10 fw-600 {{ areActiveRoutes(['home'], 'opacity-100 fw-600') }} opacity-60">{{ translate('Home') }}</span>
            </a>
            <a href="{{ route('frontend.notifications') }}" class="text-reset flex-grow-1 pb-2 pt-3 text-center">
                <span class="d-inline-block position-relative px-2">
                    <i class="las la-bell fs-20 {{ areActiveRoutes(['frontend.notifications'], 'opacity-100 text-primary') }} opacity-60"></i>
                ...
                <span class="d-block fs-10 fw-600 {{ areActiveRoutes(['frontend.notifications'], 'opacity-100 fw-600') }} opacity-60">{{ translate('Notifications') }}</span>
            </a>
            <a href="{{ route('all.messages') }}" class="text-reset flex-grow-1 pb-2 pt-3 text-center">
                <span class="d-inline-block position-relative px-2">
                    <i class="las la-comment-dots fs-20 {{ areActiveRoutes(['all.messages'], 'opacity-100 text-primary') }} opacity-60"></i>
                    @php
                ...
                <span class="d-block fs-10 fw-600 {{ areActiveRoutes(['all.messages'], 'opacity-100 fw-600') }} opacity-60">{{ translate('Messages') }}</span>
            </a>
            @if (Auth::check())
            @if (isClient() || isExpert())
            <a href="javascript:void(0)" class="text-reset flex-grow-1 mobile-side-nav-thumb pb-2 pt-3 text-center" data-toggle="class-toggle" data-target=".aiz-mobile-side-nav">
                <span class="d-block mx-auto">
                    @if (Auth::user()->photo != null)
                    <img src="{{ custom_asset(Auth::user()->photo) }}" class="rounded-circle size-20px">
                    @else
                    <img src="{{ asset('assets/frontend/default/img/avatar-place.png') }}" class="rounded-circle size-20px">
                    @endif
