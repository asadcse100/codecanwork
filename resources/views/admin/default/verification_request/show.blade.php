    ...
    <div class="layout-px-spacing">
        <div class="">
            <div class="col-md-3   layout-top-spacing">
                <h5 class="mb-md-0 h6">{{ translate('Verification Details') }}</h5>
            </div>
                        ...
                        <div class="card-body text-center">
                            ...
                            <div class="text-center">
                                @if ($user->userPackage != null && $user->userPackage->package != null)
                                    <span class="avatar avatar-square avatar-xxs"
                                        title="{{ $user->userPackage->package->name }}">
                            ...
                            <div class="mt-5 text-left">
                                <h6 class="separator mb-4 text-left"><span
                                        class="card pr-3">{{ translate('Verification') }}</span></h6>
                            ...
                            <div class="text-left mt-5">
                                <h6 class="separator mb-4 text-left"><span
                                        class="card pr-3">{{ translate('Account Information') }}</span></h6>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-6">
                                            ...
                                            @if ($verification_request->verified == 1)
                                                <a onclick="reject('{{ $verification_request->id }}')"
                                                    class="btn btn-sm btn-outline-danger">{{ translate('Reject') }}</a>
                                            @else
                                                <a onclick="accept('{{ $verification_request->id }}')"
                                                    class="btn btn-sm btn-outline-primary">{{ translate('Accept') }}</a>
                                                <a onclick="reject('{{ $verification_request->id }}')"
                                                    class="btn btn-sm btn-outline-danger">{{ translate('Reject') }}</a>
                                            @endif
                                        </div>
                                        <div class="col-6">
                                            <div class="file-preview">
                                                ...
                                                @endphp
                                                @if ($attachment != null)
                                                    <a class="d-flex justify-content-between align-items-center mt-2 file-preview-item text-reset"
                                                        href="{{ my_asset($attachment->file_name) }}" target="_blank"
                                                        download>
                                                        <div
...
@section('script')
    <script type="text/javascript">
        function accept(id) {
            $.post('{{ route('verififaction_accept') }}', {
                _token: '{{ csrf_token() }}',
                id: id
            }, function(data) {
                if (data == 1) {
        ...
        function reject(id) {
            $.post('{{ route('verififaction_reject') }}', {
                _token: '{{ csrf_token() }}',
                id: id
            }, function(data) {
                if (data == 1) {
