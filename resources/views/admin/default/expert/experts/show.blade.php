                    ...
                    <div class="widget-content widget-content-area">
                        <div class="card-body text-center">
                            <span class="avatar avatar-xxl mb-3">
                                @if ($user->photo != null)
                                    <img class="img-md" src="{{ asset('profile/photos/' . $user->photo) }}" height="200px"
                                        alt="{{ translate('photo') }}">
                                @else
                                    <img src="{{ asset('assets/frontend/default/img/avatar-place.png') }}" height="200px"
                                        alt="{{ translate('photo') }}">
                                @endif
                            ...
                            <div class="text-center">
                                @if ($user_profile->package != null)
                                    <span class="avatar avatar-square avatar-xxs"
                                        title="{{ $user_profile->package->name }}">
                                    ...
                                    @if ($user_badge->badge != null)
                                        <span class="avatar avatar-square avatar-xxs"
                                            title="{{ $user_badge->badge->name }}"><img
                                                src="{{ my_asset($user_badge->badge->icon) }}"></span>
                                    @endif
                                @endforeach
                            </div>
                            <div class="mt-5 text-left">
                                <h6 class="separator mb-4 text-left"><span
                                        class="text-success pr-3">{{ translate('Verification') }}</span></h6>
                                <p class="text-muted text-capitalize">
                                    <span>{{ translate('Email Verification') }} :</span>
                                    @if ($user->email_verified_at != null)
                                        <span class="badge badge-sm float-right badge-circle badge-success"> <i
                                                class="las la-check"></i> </span>
                                    @else
                                        <span class="badge badge-sm float-right badge-circle badge-secondary"> <i
                                ...
                                @if ($user->verifications != null)
                                    @foreach ($user->verifications as $key => $verification)
                                        <p class="text-muted text-capitalize">
                                            <span>{{ str_replace('_', ' ', $verification->type) }} :</span>
                                            @if ($verification->verified != 0)
                                                @if ($verification->type == 'identity_verification')
                                                    <span class="badge badge-sm float-right badge-circle badge-success">
                                            ...
                                            @else
                                                @if ($verification->type == 'identity_verification')
                                                    <span class="badge badge-sm float-right badge-circle badge-secondary">
                            ...
                            <div class="text-left mt-5">
                                <h6 class="separator mb-4 text-left"><span
                                        class="text-success pr-3">{{ translate('Account Information') }}</span></h6>
                                ...
                                <p class="text-muted">
                                    ...
                                    <span class="ml-2">
                                        @if ($user->address->postal_code != null)
                                    ...
                                    <span class="ml-2">
                                        @if ($user_profile->skills != null)
                                            @foreach (json_decode($user_profile->skills) as $key => $skill_id)
                                                ...
                                                @if ($skill != null)
                                                    <span
                                                        class="badge badge-inline badge-secondary">{{ $skill->name }}</span>
                                                @endif
                                            @endforeach
                                        @endif
                                    </span>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="text-left">
                                    ...
                                    <p class="text-muted"><strong>{{ translate('Bank Account Name') }}
                                            :</strong><span class="ml-2">{{ $user_account->bank_acc_name }}</span>
                                    </p>

                                    <p class="text-muted"><strong>{{ translate('Bank Account Number') }}
                                            :</strong><span class="ml-2">{{ $user_account->bank_acc_no }}</span>
                                    </p>
