                        ...
                        <tbody>
                            @foreach ($badges as $key => $badge)
                                <tr>
                                    ...
                                    <td><img class="img-md" src="{{ my_asset($badge->icon) }}" height="50px"
                                            alt="{{ translate('icon') }}"></td>
                                    <td class="text-right">
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                                            href="{{ route('client_badges_edit', encrypt($badge->id)) }}"
                                            title="{{ translate('Edit') }}">
                                            <i class="las la-edit"></i>
                                        </a>
                                        <a href="#"
                                            class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                                            data-href="{{ route('badges.destroy', $badge->id) }}"
                    ...
                    <form class="form-horizontal" action="{{ route('badges.store') }}" method="POST"
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" id="name" name="name" required
                                placeholder="{{ translate('Eg. Completed 100+ projects') }}" class="form-control" required>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control aiz-selectpicker" name="type" id="type"
                                data-show="selectShow" data-target=".min-num-type" data-placeholder="Choose ...">
