...
@if ($cancel_project->project->cancel_status == 0)
    <div>
        <form class="form-horizontal" action="{{ route('cancel-project-request.request_accepted') }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            <input type="hidden" id="project_id" name="project_id" value="{{ $cancel_project->project_id }}"
                required>
            <input type="hidden" id="cancel_by_user_id" name="cancel_by_user_id"
                value="{{ $cancel_project->requested_user_id }}" required>
