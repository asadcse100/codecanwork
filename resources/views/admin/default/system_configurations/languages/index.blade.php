...
@section('content')
    <div class="layout-px-spacing">
        <div class="col-md-3 layout-top-spacing">
            <h6>{{ translate('System Language') }}</h6>
        </div>
                                ...
                                @foreach ($languages as $key => $language)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span
                                                    class="badge badge-inline badge-secondary">{{ translate('Off') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <label class="aiz-switch aiz-switch-success mb-0">
                                                <input type="checkbox" id="language_enable.{{ $key }}"
                                                    onchange="update_language_status(this)" value="{{ $language->id }}"
                                                    @if ($language->enable == 1) checked @endif>
                                                <span></span>
                                            </label>
                                        </td>
                                        <td class="text-right">
                        ...
                        <form class="form-horizontal" action="{{ route('languages.store') }}" method="POST"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" id="name" name="name"
                                    placeholder="{{ translate('Eg. English') }}"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control aiz-selectpicker" name="code" id="code"
                                    title="Select a country code" data-live-search="true">
                            ...
                            <div class="form-group mb-3">
                                <label class="align-self-center"
                                    for="rtl">{{ translate('Is this language RTL?') }}</label>
                                <div class="">
                                    <label class="aiz-switch aiz-switch-success mb-0">
                    ...
                    <div class="card-body">
                        <form class="form-horizontal" action="{{ route('env_key_update.update') }}" method="POST"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="select2 form-control aiz-selectpicker" name="DEFAULT_LANGUAGE"
                                    data-toggle="select2" data-placeholder="Choose ...">
                                    @foreach (\App\Models\Language::where('enable', 1)->get() as $key => $language)
                                        <option value="{{ $language->code }}"
                                            @if (env('DEFAULT_LANGUAGE') == $language->code) selected @endif>{{ $language->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group mb-3 text-right">
                                <button type="submit"
                                    class="btn btn-primary">{{ translate('Save Default Language') }}</button>
                            </div>
                        </form>
                    </div> <!-- end card-body -->
                </div>
...
@section('script')
    <script type="text/javascript">
        function update_language_status(el) {
            ...
            $.post('{{ route('languages.update_language_status') }}', {
                _token: '{{ csrf_token() }}',
                id: el.value,
                status: status
            }, function(data) {
