    ...
    <div class="row">
        <div class="col-lg-8 mx-auto">
            <div class="card">
                ...
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('employees.update', $user->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group mb-3">
                            <label for="name">{{ translate('Name') }}</label>
                            <input type="text" id="name" name="name" value="{{ $user->name }}" required
                                placeholder="{{ translate('Eg. Leom Jafex') }}" class="form-control">
                        </div>
                        <div class="form-group mb-3">
                            <label for="email">{{ translate('Email') }}</label>
                            <input type="email" id="email" name="email" value="{{ $user->email }}" disabled
                                placeholder="{{ translate('Eg. email@example.com') }}" class="form-control">
                        </div>
                        <div class="form-group mb-3">
                            <label for="password">{{ translate('Password') }}</label>
                            <input type="password" id="password" name="password"
                                placeholder="{{ translate('Eg. ********') }}" class="form-control">
                        </div>
                        <div class="row">
                            <div class="col-lg-4">
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control aiz-selectpicker" name="country_id" id="country_id"
                                        ...
                                        @foreach (\App\Models\Country::all() as $key => $country)
                                            <option value="{{ $country->id }}"
                                                @if ($user->address->country_id == $country->id) selected @endif>{{ $country->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-lg-4">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" id="postal_code" name="postal_code"
                                        value="{{ $user->address->postal_code }}" required
                                        placeholder="{{ translate('Eg. 1203') }}" class="form-control">
                                </div>
                            </div>
                        </div>
                        <div class="form-group mb-3">
                            ...
                            <input type="text" id="street" name="street" value="{{ $user->address->street }}"
                                required placeholder="{{ translate('Eg. Street') }}" class="form-control">
                        </div>
                        <div class="form-group mb-3">
                            <label for="phone">{{ translate('Phone') }}</label>
                            <input type="number" id="phone" name="phone" value="{{ $user->address->phone }}"
                                required placeholder="{{ translate('Eg. 015XXXXXXXX') }}" class="form-control">
                        </div>
                        <div class="form-group mb-3">
                            <label for="designation">{{ translate('Designation') }}</label>
                            <select class="select2 form-control aiz-selectpicker" name="designation" id="designation"
                                data-toggle="select2" data-placeholder="Choose ...">
                                @foreach (\App\Models\Role::all() as $key => $role)
                                    @if ($role->id != '1' && $role->id != '2' && $role->id != '3')
                                        <option
                                            value="{{ $role->id }}"@if (\App\Models\UserRole::where('user_id', $user->id)->first()->role_id == $role->id) selected @endif>
                                            {{ $role->name }}</option>
                                    @endif
                                @endforeach
                            </select>
